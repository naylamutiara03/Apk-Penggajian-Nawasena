...
if (!isset($_SESSION['html_laporan'])) {
    echo "Data tidak tersedia untuk didownload.";
    exit;
}

$htmlBody = $_SESSION['html_laporan'];

// Ambil periode dari HTML
preg_match('/<strong>Bulan:<\/strong>\s*(.*?)<\/p>/', $htmlBody, $periodeMatch);
$periode = isset($periodeMatch[1]) ? trim($periodeMatch[1]) : 'Periode';
...
<h2 class="title">LAPORAN GAJI BULAN ' . strtoupper($periode) . '</h2>
';
...
$namaFile = 'Laporan-Gaji-' . str_replace(' ', '_', $periode) . '.pdf';
